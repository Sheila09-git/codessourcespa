                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        <?php foreach ($list as $users):
                            $isDeleted = ($users['is_delete'] == 1);
                        ?>

    <tr class="<?php echo $isDeleted ? 'row-deleted' : ''; ?>">
        ...
        <td class="text-end">
    <?php if ($isDeleted): ?>
        <button class="btn btn-primary" disabled>Modifier</button>
    <?php else: ?>
        <a href="update_client.php?id=<?= $users['id_utilisateur'] ?>" class="btn btn-primary">Modifier</a>
    <?php endif; ?>

    <button class="btn btn-danger" 
            data-bs-toggle="modal" 
            data-bs-target="#modalDelete<?php echo $users['id_utilisateur']; ?>"
            <?php echo $isDeleted ? 'disabled' : ''; ?>>Supprimer</button>

    <?php if ($users['is_blocked'] == 0): ?>
        <button class="btn btn-warning" 
                data-bs-toggle="modal" 
                data-bs-target="#modalBlock<?= $users['id_utilisateur'] ?>"
                <?php echo $isDeleted ? 'disabled' : ''; ?>>Bloquer</button>
    <?php else: ?>
        <button class="btn btn-success" 
                data-bs-toggle="modal" 
                data-bs-target="#modalUnblock<?= $users['id_utilisateur'] ?>"
                <?php echo $isDeleted ? 'disabled' : ''; ?>>Débloquer</button>
    <?php endif; ?>
        ...
        <div class="modal-dialog">
            <div class="modal-content text-start">
                ...
                <div class="modal-body">
                    Êtes-vous sûr de vouloir bloquer <strong><?php echo htmlspecialchars($users['username']); ?></strong> ?
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
                    <a href="processing.php?action=block&id=<?php echo $users['id_utilisateur']; ?>" class="btn btn-warning">Confirmer</a>
                </div>
            </div>
        </div>
    </div>

    <div class="modal fade" id="modalUnblock<?= $users['id_utilisateur']; ?>">
        <div class="modal-dialog">
            <div class="modal-content text-start">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    Voulez-vous redonner l'accès à <strong><?= htmlspecialchars($users['username']); ?></strong> ?
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
                    <a href="processing.php?action=unblock&id=<?= $users['id_utilisateur']; ?>" class="btn btn-success">Confirmer le déblocage</a>
                </div>
            </div>
        </div>
    </div>
        ...
        <?php
          $requete = $pdo->query("SELECT candidatures.*, utilisateur.email 
            ...
            <tr>
                ...
                <td>
                    <a href="uploads/<?= htmlspecialchars($ligne['cv_path']); ?>"target="_blank"class="btn btn-sm btn-outline-danger">Voir CV</a>
                </td>
                <td>
                    <a href="uploads/<?= htmlspecialchars($ligne['lm_path']); ?>"target="_blank"class="btn btn-sm btn-outline-primary">Voir LM</a>
                </td>
